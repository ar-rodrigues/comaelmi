...
export default function Product({productData, deleteProduct, error, setError}) {
  ...
  const handleChange = (e) => {
   ...
   useEffect(() => {
    ...
    confirmationRef.current && !confirmationRef.current.contains(event.target) ? setShowConfirmation(false) : null
    });
    return () => {
...
// BUTTONS
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            ...
            {
              showConfirmation && <button onClick={()=>setShowConfirmation(false)} className="flex-auto btn btn-secondary btn-sm btn-block text-white" >
            No
          </button>
            }
          ...
          <button
            ...
            onClick={() => !edit ? (setEdit(true), setShowConfirmation(false)): (saveProduct(id, product, setEdit, setError), setShowConfirmation(false))}
          >
            Editar
          </button>
          {
            error && 
            <Alert message="Error: Producto no fue editado"/>
          }
        </div>
